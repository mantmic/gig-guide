...
( select
  ...
  , coalesce ( thebrag_geocoded.geocode_location_id, bandcamp_geocoded.geocode_location_id ) as geocode_location_id
from
  thebrag_geocoded
  full outer join
  bandcamp_geocoded
...
select
    to_hex ( {{ dbt_utils.surrogate_key('geocode_location_id', 'thebrag_venue_id', 'bandcamp_venue_id') }} ) as venue_id
  , *
from
  results_combined
